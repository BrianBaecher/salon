...
CUT_PRC() {
    ...
    if [[ -z $GET_CUSTOMER_NAME ]]; then
        ...
        echo -e "\nWelcome $CUSTOMER_NAME, at what time would you like your cut?"
        read SERVICE_TIME
        #get custmer id
        CUSTOMER_ID=$($PSQL "SELECT customer_id FROM customers WHERE phone='$CUSTOMER_PHONE'")
        #insert appointment (for cut)
        INSERT_APPOINTMENT_CUT=$($PSQL "INSERT INTO appointments (customer_id, service_id, time) VALUES ($CUSTOMER_ID, 1, '$SERVICE_TIME')")
